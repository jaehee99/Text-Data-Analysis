...
In the Jungle graph, we can see that at the end the values drop off so suddenly. 
This is because of the index and linenumber. To be specific: 
In the Jungle there are 28 index, 14027 line numbers. 
500 * 28 = 14000, 14027 - 14000 so there are 27 lines left.
In the Quest Silver book there are 28 index, 14489 line numbers. 
500 * 28 = 14000, 14489 - 14000 so there are 489 lines left.
...
1. What are some key ideas from this podcast relevant to text sentiment analysis?
David discusses “Generating Sentiment-Preserving Fake Online Reviews” with Neural Language Models and their Machine learning based Detection. He researches on privacy preserving, and also researches on detecting fake reviews. He talks about the relationship between Natural Language Processing and faking text. Also, he mentions sentiment online neural network reviews. Furthermore, he says that the tough thing in speech transcripts is the people’s different speaking style, and there are ways, speech transcript, style of the writing. He says that customer’s privacy such as the person’s information, critical number, phone number have to be preserved as well. He also generates sentiment using natural language models. Fake reviews are common, but the important thing is that the model for generating fake reviews is getting better so it is making humans harder to detect. Why this review system is important is because it influences the customers’ decision, it can attack the competitor and it also influences the politics. Fake reviews can be used to influence the customers and to increase the company’s profit. People are not only influenced by the review but also affected by the volume of the reviews. Review systems and politics, twitter can change people’s decisions. It is getting hard for humans to detect which one is human or non-human however, there is a way to distinguish. For example, if humans wrote the review, it uses more diverse worlds used in the real world, whereas, if it is generated by algorithms it is using frequent words. Also, he recommends to see the average rating compared to reading the reviews. Lastly, he talks about sentiment preserving, examples of the positive or negative reviews, generating more reviews, preserving the context and paraphrasing the review, and so on.

2. How do you think the ideas discussed may be relevant in your future work?
Since the predicting models and a lot of models are improved, the important thing is the ethics of the projects. In my future work, it is better to use the good skills incorporated into making the model better, and based on the reviews we have to use it for improving customer’s satisfaction not in business intention to attract people. This sentiment analysis can be used to know how people think about the issue or know how people feel about the product. It should be used ethically to not abuse it. If there were no fake reviews, then there wouldn’t have to be a fake review detection program. I think instead of knowing how to detect fake reviews, it is better to prevent fake reviews generating models at policy level. So, in my future work, I will try to use this sentiment analysis in more people-based thinking, for example, how to make the product better based on people’s review. 
